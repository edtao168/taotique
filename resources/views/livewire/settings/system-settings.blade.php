    ...
    <x-card title="系統參數設定" shadow separator>
        ...
        <x-form wire:submit="save">
            ...
            <div class="md:hidden space-y-4">
                ...
                {{-- 核心流程卡片 --}}
                <x-card title="核心流程" subtitle="Core Settings" shadow>
                    <div class="space-y-3">
                        <x-checkbox label="允許負庫存出貨" 
                            hint="開啟後允許庫存為負數時仍可出貨"
                            wire:model="payload.allow_negative_stock" />
                ...
                {{-- 單據編碼卡片 --}}
                <x-card title="單據編碼" subtitle="Numbering Rules" shadow>
                    <div class="space-y-3">
                        <x-input label="採購單前綴" 
                            wire:model="payload.po_prefix" 
                            placeholder="例如: PO-" />
                ...
                {{-- 財務設定卡片 --}}
                <x-card title="財務設定" subtitle="Finance" shadow>
                    <div class="space-y-3">
                        <x-input label="營業稅率 (%)" 
                            wire:model="payload.tax_rate" 
                            type="number" 
                ...
                {{-- 顯示設定卡片 --}}
                <x-card title="顯示設定" subtitle="Display" shadow>
                    <div class="space-y-3">
                        <x-checkbox label="顯示庫存成本" 
                            hint="開啟後在庫存列表顯示成本金額"
                            wire:model="payload.show_cost_fields" />
                ...
                {{-- 安全性設定卡片 --}}
                <x-card title="安全性設定" subtitle="Security" shadow>
                    <div class="space-y-3">
                        <x-checkbox label="記錄操作日誌" 
                            hint="記錄所有使用者的操作行為"
                            wire:model="payload.enable_audit_log" />
                ...
                {{-- 整合設定卡片 --}}
                <x-card title="整合設定" subtitle="Integration" shadow>
                    <div class="space-y-3">
                        <x-checkbox label="啟用庫存低於安全量警報" 
                            hint="當庫存量低於安全庫存時發出警報通知"
                            wire:model="payload.stock_alert_enabled" />
