...
<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3">
            <x-card title="商品明細" shadow separator>
                {{-- 條碼掃描按鈕 (可選) --}}
                <div class="mb-4">
                    <x-button label="掃描條碼" icon="o-camera" class="btn-outline btn-sm" 
                        wire:click="$set('showScanner', true)" />
                </div>
    ...
    <x-modal wire:model="showScanner" title="掃描條碼" separator>
        <x-input placeholder="請掃描或輸入條碼" wire:model="scannedBarcode" 
            wire:keydown.enter="handleScannedBarcode(scannedBarcode)" />
        <x-slot:actions>
            <x-button label="關閉" @click="$wire.showScanner = false" />
        </x-slot:actions>
    </x-modal>
</div>