    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-24">
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <x-card separator shadow class="bg-base-100/60 backdrop-blur border-t-4 border-primary">
                ...
                {{-- PC 端標頭（手機隱藏） --}}
                <div class="hidden lg:grid grid-cols-12 gap-4 px-4 mb-2 text-sm font-bold opacity-60">
                    <div class="col-span-4">商品資訊</div>
                    <div class="col-span-2">收貨倉庫</div>
                    <div class="col-span-2 text-right">單價 ({{ $currency }})</div>
                    <div class="col-span-1 text-right">數量</div>
                    <div class="col-span-3 text-right text-primary">小計</div>
                </div>

                <div class="space-y-3">
                    @forelse($items as $index => $item)
                        <div wire:key="purchase-row-block-{{ $index }}-{{ $item['product_id'] ?? 'new' }}">
                            <x-purchase-row 
                                :$index 
                                :$item 
                                :$warehouses 
                ...
                <x-slot:actions>
                    <x-button 
                        label="手動新增一行商品" 
                        icon="o-plus-circle" 
                        class="btn-ghost btn-sm w-full border-dashed border-2 hover:border-primary hover:text-primary" 
                        wire:click="addRow" 
                    />
                </x-slot:actions>
            </x-card>
