        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 gap-3 scale-[0.8] origin-top-left w-[125%]">
            ...
            @foreach($new_photos as $index => $photo)
                <div class="relative group aspect-square">
                    ...
                    <div wire:loading wire:target="temp_photos" class="absolute inset-0 flex items-center justify-center bg-white/30 rounded-lg">
        ...
        <div 
            x-data="{ 
                ...
                handleDrop(e) {
                    ...
                    if (files.length > 0) {
                        // 使用 Livewire v3 的 uploadMultiple 直接上傳
                        $wire.uploadMultiple('temp_photos', files);
                    }
                }
            }"
            ...
            class="p-4 border-2 border-dashed rounded-xl text-center bg-base-200 hover:border-primary transition-colors cursor-pointer group select-none border-base-300"
        >
            ...
            <span class="text-[10px] text-gray-400 block mt-1">可多次拖曳，自動累積</span>
        </div>
        
        @if($errors->has('new_photos.*') || $errors->has('temp_photos.*'))
            <div class="mt-2 text-xs text-error">
                @foreach($errors->get('new_photos.*') as $error)
                    {{ $error[0] }}<br>
                @endforeach
                @foreach($errors->get('temp_photos.*') as $error)
                    {{ $error[0] }}<br>
                @endforeach
            </div>
        @endif
