    ...
    protected function resolveSaleDomainAccount(Model $source, string $type, ?string $subType): string
{
	...
	{
		// 🎯 核心：當來源是 cost_amount，直接在 Service 內跑明細成本高精度加總
		if ($amountSource === 'cost_amount') {
			if (!isset($source->items)) {
				$source->load('items'); // 確保防範 N+1 查詢並動態載入明細
			}

			$totalCost = '0.0000';
			foreach ($source->items as $item) {
				$itemCost = (string)($item->cost ?? 0);
				$itemQty  = (string)($item->quantity ?? 0);
				$subCost  = bcmul($itemCost, $itemQty, 4); // 成本 * 數量
				$totalCost = bcadd($totalCost, $subCost, 4); // 累加總成本
			}

			return $totalCost;
		}

		// 其他標準單據欄位對應（對齊您現有的資料庫命名）
		$val = match ($amountSource) {
			'total_amount', 'subtotal'      => $source->subtotal ?? $source->total_amount ?? 0,
			'tax_amount'                    => $source->tax_amount ?? 0,
			'grand_total', 'customer_total' => $source->grand_total ?? $source->total_amount ?? 0,
			'platform_fee'                  => $source->platform_fee ?? $source->fee_amount ?? 0,
			'commission'                    => $source->commission ?? 0,
			'seller_discount'               => $source->seller_discount ?? $source->discount_amount ?? 0,
			'shipping_fee_platform'         => $source->shipping_fee_platform ?? 0,
			'freight_amount'                => $source->freight_amount ?? $source->shipping_fee ?? 0,
			'total_fees'                    => $source->total_fees ?? 0,
